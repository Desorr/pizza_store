...
# Для отмены добавления баннера
@admin_router.message(StateFilter('*'), Command("стоп"))
@admin_router.message(StateFilter('*'), F.text.casefold() == "стоп")
async def cancel_banner_handler(message: types.Message, state: FSMContext) -> None:
    current_state = await state.get_state()
    if current_state == AddBanner.image:
        await state.clear()
        await message.answer("Процесс добавления/изменения баннера остановлен.", reply_markup=ADMIN_KB)
...
async def add_banner2(message: types.Message, state: FSMContext):
    await message.answer("Отправьте фото баннера или стоп")
...
# Изменение продукта в БД при нажатии на инлайн кнопку
...
async def cancel_handler(message: types.Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer("Процесс добавления нового товара отменен.", reply_markup=ADMIN_KB)
...
async def category_choice(callback: types.CallbackQuery, state: FSMContext , session: AsyncSession):
    ...
    if int(callback.data) in category_ids:
        ...
        await callback.message.answer(f"Введите цену товара")
...
async def add_price(message: types.Message, state: FSMContext):
    if message.text == "." and AddProduct.product_for_change:
        ...
        try:
            ...
            if price <= 0:
                raise ValueError("Ввели цену 0.")
        except ValueError as e:
            await message.answer(f"Ошибка: {e}. Введите корректную цену, больше 0.")
            return
